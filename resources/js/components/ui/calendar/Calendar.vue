...
<template>
  <DefineMonthTemplate v-slot="{ date }">
    <div class="**:data-[slot=native-select-icon]:right-2">
      <div class="relative border-3 border-foreground bg-background shadow-[3px_3px_0px_hsl(var(--shadow-color))]">
        <div class="absolute inset-0 flex h-full items-center text-sm pl-2 pointer-events-none">
          {{ formatter.custom(toDate(date), { month: 'short' }) }}
        </div>
        <NativeSelect
          class="text-xs h-9 pr-7 pl-2 text-transparent relative font-bold uppercase tracking-wide bg-transparent border-0 focus-visible:ring-0"
        ...
        <CalendarGridBody>
          <CalendarGridRow v-for="(weekDates, index) in month.rows" :key="`weekDate-${index}`" class="mt-2 w-full">
            <CalendarCell
              ...
              :date="weekDate"
            >
              <slot name="calendar-cell" :day="weekDate" :month="month.value">
                <CalendarCellTrigger
                  :day="weekDate"
                  :month="month.value"
                />
